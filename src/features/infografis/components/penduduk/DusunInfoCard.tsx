...
export function DusunInfoCard({
  ...
  return (
    <article className="hero-reveal group relative pt-3 sm:pt-4" style={{ animationDelay: `${delayMs}ms` }}>
      ...
      <span className="absolute right-1.5 top-0.5 z-30 inline-flex items-center gap-1 rounded-full bg-[#a2d6f9] px-1.5 py-0.5 text-[9px] font-bold text-white shadow-[0_4px_12px_rgba(0,0,0,0.2)] border border-white/20 sm:gap-1.5 sm:px-3 sm:py-1 sm:text-[13px]">
        <span className="h-1 w-1 animate-pulse rounded-full bg-white sm:h-1.5 sm:w-1.5" />
        {persentase}%
      </span>

      <div className="relative z-10 min-h-28 overflow-hidden rounded-[18px] border border-white/10 bg-linear-to-br from-white/15 via-white/5 to-transparent px-3.5 pb-3.5 pt-5 shadow-[0_20px_40px_rgba(0,0,0,0.15)] backdrop-blur-sm transition-all duration-300 group-hover:border-white/25 group-hover:bg-white/10 sm:min-h-38 sm:rounded-[24px] sm:px-5 sm:pb-5 sm:pt-7">
        ...
        <div className="relative">
          <h3 className="font-timeless leading-[0.85] text-white">
            <span className="block text-[22px] opacity-70 sm:text-[38px]">Dusun</span>
            <span className="mt-1 block text-[24px] font-bold tracking-tight sm:text-[34px]">{nama.replace("Dusun ", "")}</span>
          </h3>
          <p className="mt-1.5 line-clamp-2 text-[10px] leading-relaxed text-white/70 sm:mt-3 sm:line-clamp-none sm:text-[13px]">{deskripsi}</p>
        </div>
      </div>
    </article>
  );
